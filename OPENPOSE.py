...
# RULA Score
absolute_path_rula = r"C:\Users\Siripuram Laya\Downloads\Biomechanics-Ai_ntl.ipynb-main\Biomechanics-Ai_ntl.ipynb-main\Rula_score\\"

# Reading RULA CSV files with absolute paths
table_a_rula = pd.read_csv(absolute_path_rula + 'TableA.csv')
table_b_rula = pd.read_csv(absolute_path_rula + 'TableB.csv')
table_c_rula = pd.read_csv(absolute_path_rula + 'TableC.csv')

# Continue with your data processing using these RULA DataFrames

# REBA Score
absolute_path_reba = r"C:\Users\Siripuram Laya\Downloads\Biomechanics-Ai_ntl.ipynb-main\Biomechanics-Ai_ntl.ipynb-main\Reba_score\\"

# Reading REBA CSV files with absolute paths
table_d_reba = pd.read_csv(absolute_path_reba + 'TableA.csv')
table_e_reba = pd.read_csv(absolute_path_reba + 'TableB.csv')
table_f_reba = pd.read_csv(absolute_path_reba + 'TableC.csv')

# Continue with your data processing using these REBA DataFrames
...
profile=""
import pandas as pd
tablea=pd.read_csv("C:\\Users\\Siripuram Laya\\Downloads\\Biomechanics-Ai_ntl.ipynb-main\\Biomechanics-Ai_ntl.ipynb-main\\Rula_score\\TableA.csv")
tableb=pd.read_csv("C:\\Users\\Siripuram Laya\\Downloads\\Biomechanics-Ai_ntl.ipynb-main\\Biomechanics-Ai_ntl.ipynb-main\\Rula_score\\TableB.csv")
tablec=pd.read_csv("C:\\Users\\Siripuram Laya\\Downloads\\Biomechanics-Ai_ntl.ipynb-main\\Biomechanics-Ai_ntl.ipynb-main\\Rula_score\\TableC.csv")
tablea1=pd.read_csv("C:\\Users\\Siripuram Laya\\Downloads\\Biomechanics-Ai_ntl.ipynb-main\\Biomechanics-Ai_ntl.ipynb-main\\Reba_score\\TableA.csv")
tableb1=pd.read_csv("C:\\Users\\Siripuram Laya\\Downloads\\Biomechanics-Ai_ntl.ipynb-main\\Biomechanics-Ai_ntl.ipynb-main\\Reba_score\\TableB.csv")
tablec1=pd.read_csv("C:\\Users\\Siripuram Laya\\Downloads\\Biomechanics-Ai_ntl.ipynb-main\\Biomechanics-Ai_ntl.ipynb-main\\Reba_score\\TableC.csv")
...
output_folder = r"C:\Users\Siripuram Laya\Desktop\frames"

data = {'frame_number': [], 'RULA': [], 'REBA': [], 'Warning': []}
df = pd.DataFrame(data)

def image_pose_estimation(name, frame_number, fps):
    img = cv2.imread(name)
    ...
    image_path = os.path.join(output_folder, f"output_{frame_number}.png")
    print(f"Saving image to: {image_path}")
    
    cv2.imwrite(image_path, img)
    print("Image saved successfully")

    df.loc[count] = [count, rula, reba, warning_message]
...
def video_pose_estimation(name):
    ...
    if not cap.isOpened():
        print(f"Failed to open video file: {name}")
        return
    ...
    while True:
        ...
        if img is None:
            print("Empty frame received from the video.")
            break
        ...
        image_path = os.path.join(output_folder, f"output_{count}.png")
        print(f"Saving image to: {image_path}")
        cv2.imwrite(image_path, img)
        print("Image saved successfully")
        ...
        if (rula != "NULL") and (reba != "NULL"):
            warning_message = ""
            if int(rula) > 3:
                warning_message += "Posture not proper in upper body. "
            if int(reba) > 4:
                warning_message += "Posture not proper in your body."

            variable1.set("Rapid Upper Limb Assessment Score : " + rula + " " + warning_message)
            variable2.set("Rapid Entire Body Score : " + reba + " " + warning_message)
    ...
    cap.release()
    cv2.destroyAllWindows()
    df.to_excel(os.path.join(output_folder, r"C:\Users\Siripuram Laya\Downloads\score_warning.xlsx"), index=False)
...
def browsefunc():
    ...
    print("Selected file:", filename)  # Add this line for debugging
    ...
    if filename:
        ...
        if mimestart == 'video':
            ...
            image_pose_estimation(str(filename), 0, 20)  # Assuming 20 fps for images
        else:
            messagebox.showerror("Error", "Invalid file type. Please select a valid image or video file.")
    else:
        messagebox.showerror("Error", "No file selected.")

b1 = Button(root, text="input any video file for analysis", font=40, command=browsefunc).place(relx=.5, rely=.2, anchor=N)
b1 = Button(root, text="Choose Live Posture Analysis using webcam", font=40, command=webcam).place(relx=.5, rely=.4,
                                                                                                anchor=N)
root.mainloop()
